CREATE SCHEMA `bootstrap_queue` DEFAULT CHARACTER SET utf8 ;

CREATE TABLE `bootstrap_queue`.`User` (
...
CREATE TABLE `bootstrap_queue`.`Profile` (
  `Id` INT NOT NULL,
  `Description` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`Id`),
  UNIQUE INDEX `Description_UNIQUE` (`Description` ASC) VISIBLE);

CREATE TABLE `bootstrap_queue`.`Country` (
  `Id` INT NOT NULL,
  `Description` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`Id`),
  UNIQUE INDEX `Description_UNIQUE` (`Description` ASC) VISIBLE);

ALTER TABLE `bootstrap_queue`.`User` 
...
ADD CONSTRAINT `IdCountry`
  ...
  REFERENCES `bootstrap_queue`.`Country` (`Id`)
  ON DELETE NO ACTION
  ON UPDATE NO ACTION;
...
ADD CONSTRAINT `IdProfile`
  ...
  REFERENCES `bootstrap_queue`.`Profile` (`Id`)
  ON DELETE NO ACTION
  ON UPDATE NO ACTION;

INSERT INTO `bootstrap_queue`.`Country` (`Id`, `Description`) VALUES 
  ('1', 'Kanto'),
  ('2', 'Johto'),
  ('3', 'Hoenn'),
  ('4', 'Sinnoh'),
...
INSERT INTO `bootstrap_queue`.`Profile` (`Id`, `Description`) VALUES 
  ('1', 'Administrator'),
  ('2', 'Regular');

INSERT INTO `bootstrap_queue`.`User` (`IdProfile`, `IdCountry`, `CreatedBy`, `Name`, `Email`, `Document`, `Birthdate`, `Active`) VALUES 
